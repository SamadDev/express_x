...
class OrderApiService {
  ...
  static Future<OrderHistoryResponse> getOrderHistory({
    ...
    try {
      ...
      if (response != null && response['data'] != null) {
        ...
        print('⚠️ No data in response or response is null, falling back to mock data');
  ...
  static Future<Order?> createOrderFromBagItems(List<BagItem> items) async {
    try {
      ...
      print('📦 Order data: ${orderData.keys.length} fields, ${imageFiles.length} images');
      
      // Debug: Print token info
      print('🔐 Token present: ${token != null && token.isNotEmpty}');
      print('🔐 Token length: ${token?.length ?? 0}');
      print('🔐 Token preview: ${token != null ? token.substring(0, token.length.clamp(0, 20)) : "N/A"}...');
      ...
      if (imageFiles.isNotEmpty) {
        ...
        if (response.statusCode == 302) {
          print('🔄 Received 302 redirect - server authentication or configuration issue');
          ...
          if (data['order'] != null) {
            ...
            if (data['data'] is Map<String, dynamic>) {
              return Order.fromJson(data['data']);
            } else if (data['data'] is List && data['status'] != null && data['message'] != null) {
              // Success response with empty data array - create mock order
              print('✅ Success response with empty data array - creating mock order');
              // Will fall through to create mock order
            }
          } else if (data['status'] != null && data['message'] != null) {
            // Success response with status/message but no order data
            print('✅ Success response with status/message - creating mock order');
            // Will fall through to create mock order
          }
        }
      } else {
        ...
        if (response != null) {
          if (response['order'] != null) {
            ...
            if (response['data'] is Map<String, dynamic>) {
              return Order.fromJson(response['data']);
            } else if (response['data'] is List && response['status'] != null && response['message'] != null) {
              // Success response with empty data array - create mock order
              print('✅ Success response with empty data array - creating mock order');
              // Will fall through to create mock order
            }
